function submitForm() {
  ...
  const languages = [];
  document.querySelectorAll('input[name="languages[]"]:checked').forEach(function(language) {
    languages.push(language.value);
  });
  
  const form = document.querySelector('form'); // выбираем форму
  form.addEventListener('submit', submitForm); // привязываем функцию submitForm к событию "submit" формы
  
  // Створюємо таблицю з даними форми
